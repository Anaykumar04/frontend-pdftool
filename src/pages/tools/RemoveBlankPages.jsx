...
export default function RemoveBlankPages() {
  ...
  const handleProcess = async () => {
    ...
    try {
      const res = await pdfApi.removeBlankPages(file)
      setResult(res.data)
    ...
    <div className="tool-page">
      <div className="container" style={{ maxWidth: 720 }}>
        <div className="tool-page-header">
          ...
          <p>Automatically detect and remove all blank or empty pages from your PDF.</p>
        </div>
        ...
        {result ? <ResultPanel result={result} onReset={reset} /> : (
          <>
            <DropZone onFiles={onFiles} accept={{ 'application/pdf': ['.pdf'] }} multiple={false}
              ...
              hint="Upload a PDF to remove blank pages"
            ...
            {file && (
              <div className="options-panel">
                ...
                <button onClick={handleProcess} disabled={loading}
                  ...
                  {loading ? <><span className="spinner" style={{ width: 18, height: 18, borderWidth: 2 }} /> Processing...</> : '🧹 Remove Blank Pages'}
                </button>
              </div>
            )}
          </>
